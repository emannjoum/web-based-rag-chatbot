...
def execute_rag_pipeline(actual_query, selected_model, search_method):
    ...
    if not new_sources: 
        with st.spinner("Refining guidance..."):
            fallback_msg = generate_dynamic_fallback(actual_query, detected_lang, selected_model)
    ...
    with st.spinner("Consulting Altibbi AI..."):
        ...
        try:
            if selected_model == "GPT-4o mini":
                ...
                # Check if this pipeline was called silently via an OCR image extraction 
                if st.session_state.messages and "تم التعرف على الدواء" in st.session_state.messages[-1]["content"]:
                     msgs.append({"role": "user", "content": f"Please provide medical information about: {actual_query}"})
                ...
                # Safely construct strictly alternating history for Gemini
                history = []
                ...
                # Setup the final user prompt to invoke generation
                if history and history[-1]["role"] == "user":
                    last_user_content = history.pop()["parts"][0]
                    if st.session_state.messages and "تم التعرف على الدواء" in st.session_state.messages[-1]["content"]:
                         last_user_content += f"\n\nPlease provide medical information about: {actual_query}"
                else:
                    last_user_content = f"Please provide medical information about: {actual_query}"
                
                model = genai.GenerativeModel(model_name="gemini-2.5-flash-lite", system_instruction=sys_prompt)
                chat = model.start_chat(history=history)
                ai_reply = chat.send_message(last_user_content).text
            ...
            eval_thread = threading.Thread(
                ...
                args=(db, assistant_msg_id, actual_query, ai_reply, new_context)
            )
            eval_thread.start()

            log_to_file_and_terminal(actual_query, last_question, raw_urls, new_sources, new_context, ai_reply)
...
if user_input := st.chat_input("Ask Altibbi... (Text or upload an image)", accept_file=True, file_type=["png", "jpg", "jpeg"]):
    
    if user_input.files:
        uploaded_file = user_input.files[0]
        image_bytes = uploaded_file.getvalue()
        text_addon = f" (Message: '{user_input.text}')" if user_input.text else ""
        user_msg = f"[Uploaded an Image: {uploaded_file.name}]{text_addon}"
        ...
        if "unsupported" in image_type or image_type not in ["report", "drug"]:
            ...
            new_id, _ = db.log_interaction(
                ...
                metadata={"model": selected_model, "content_type": "report", "chat_title": "Report Analysis"}
            )
            if st.session_state.current_chat_id is None:
                st.session_state.current_chat_id = new_id
            st.rerun()
            ...
            with st.spinner("Extracting medication name from image..."):
                extracted_drug_name = extract_drug_name_from_image(image_bytes, selected_model)
            
            st.session_state.messages.append({"role": "assistant", "content": f"**تم التعرف على الدواء:** {extracted_drug_name}", "sources": {}})
            with st.chat_message("assistant"):
                st.markdown(f"**تم التعرف على الدواء:** {extracted_drug_name}\n\n*جاري البحث في الطبي...*")
            
            execute_rag_pipeline(extracted_drug_name, selected_model, search_method)
        ...
        execute_rag_pipeline(user_query, selected_model, search_method)