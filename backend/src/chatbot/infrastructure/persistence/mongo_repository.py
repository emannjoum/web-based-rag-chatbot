...
class MongoChatRepository:
    ...
    def log_interaction(
        ...
        if session_id and not self.session_belongs_to_user(session_id, user_id):
            raise PersistenceError("Session not found or access denied.")
    ...
    def get_all_history(self, user_id: int, limit: int = 20) -> list[dict[str, Any]]:
        try:
            pipeline = [
                {"$match": {"is_delete": {"$ne": 1}, "user_id": user_id}},
                {"$sort": {"timestamp": -1}},
                {
                    "$group": {
                        "_id": "$session_id",
    ...
    def get_chat_by_id(self, session_id: str, user_id: int) -> list[dict[str, Any]]:
        if not self.session_belongs_to_user(session_id, user_id):
            return []

        return list(
            self._messages_col.find(
                {"session_id": session_id, "user_id": user_id, "is_delete": {"$ne": 1}}
            ).sort("timestamp", 1)
        )
    ...
    def session_belongs_to_user(self, session_id: str, user_id: int) -> bool:
        doc = self._messages_col.find_one(
            {"session_id": session_id, "user_id": user_id, "is_delete": {"$ne": 1}},
            {"_id": 1},
        )
