...
class SidebarComponent:
    ...
    def _render_history(self) -> None:
        ...
        for chat in self._repository.get_all_history(limit=15) or []:
            ...
            if col1.button(chat_label, key=f"load_{chat['_id']}", use_container_width=True):
                st.session_state.messages = self._repository.get_chat_by_id(str(chat["_id"])) or []
                st.session_state.current_chat_id = str(chat["_id"])
                st.rerun()

            with col2.popover("⋮"):
                if st.button("Delete", key=f"del_{chat['_id']}", type="primary"):
                    self._repository.delete_chat(str(chat["_id"]))
